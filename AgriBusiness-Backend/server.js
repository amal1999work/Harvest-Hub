...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (category === 'farmer' && req.files && req.files.image) {
  ...
  try {
    ...
    if (user) {
      ...
      if (user.password === userPassword) {
        ...
        const userDetails = await db.collection('register').findOne({ userId: user._id });
        ...
        if (userDetails) {
          ...
          console.log('User details not found in register collection for userId:', user._id);
...
app.get('/newfarmer', async (req, res) => {
  try {
    ...
    const logins = await db.collection('login').find({
      _id: { $in: userIds },
      userStatus: { $in: [2, '2'] }
    }).toArray();
    ...
    if (logins.length === 0) {
      ...
      const loginsWithoutStatus = await db.collection('login').find({ _id: { $in: userIds } }).toArray();
...
app.post('/addcategory', async (req, res) => {
  try {
    ...
    const existingCategory = await db.collection('category').findOne({ category });
...
app.post('/deletecategory', (req, res) => {
  let delId = req.body.id
  database.then((db) => {
    db.collection('category').deleteOne({ _id: new mongodb.ObjectId(delId) })
      .then((result) => {
        res.json(result)
      }
      )
...
app.post('/findcategory', (req, res) => {
  ...
  database.then((db) => {
    db.collection('category').findOne({ _id: new mongodb.ObjectId(findId) })
      .then((result) => {
        res.json(result);
      })
      .catch((error) => {
...
function updateCategoryInDb(findId, categories, res) {
  database.then((db) => {
    db.collection('category').updateOne({ _id: new mongodb.ObjectId(findId) }, { $set: categories })
      .then((result) => {
        console.log(result);
        res.json("success");
      })
...
app.post('/addproduct', (req, res) => {
  ...
  picture.mv(uploadPath, function (err) {
    ...
    database.then((db) => {
      db.collection('product').insertOne(products).then((result) => {
        console.log(result);
        res.json("success");
      }).catch((error) => {
...
app.post('/viewproduct', async (req, res) => {
  ...
  try {
    const db = await database;
    const productresult = await db.collection('product').aggregate([
...
// console.log(productresult);
    ...
    const farmers = await db.collection('register').find({
...
app.post('/deleteProduct', (req, res) => {
  let delId = req.body.id
  database.then((db) => {
    db.collection('product').deleteOne({ _id: new mongodb.ObjectId(delId) })
      .then((result) => {
        res.json(result)
      }
      )
...
app.post('/findproduct', (req, res) => {
  ...
  database.then(async (db) => {
    ...
    var productresult = await db.collection('product').findOne({ _id: new mongodb.ObjectId(findId) })
...
app.post('/updateproduct', (req, res) => {
  ...
  function updateProductInDb(findId, products, res) {
    database.then((db) => {
      db.collection('product').findOne({ _id: new mongodb.ObjectId(findId) })
...
const db = await database
    try {
        ...
        res.status(200).json({ message: 'Cultivation details added successfully', id: result.insertedId });
    } catch (error) {
...
app.post('/viewcultivation', async (req, res) => {
  ...
  try {
    ...
    const cultivationDataStep1 = await db.collection('cultivation').find({ farmerid: new mongodb.ObjectId(farmerid) }).toArray();
    console.log("Step 1: Cultivation Data Match:", cultivationDataStep1);

    const cultivationDataStep2 = await db.collection('cultivation').aggregate([
      { $match: { farmerid: new mongodb.ObjectId(farmerid) } },
      { $addFields: { subcategoryId: { $toObjectId: "$subcategory" } } }
    ]).toArray();
    console.log("Step 2: Cultivation Data with ObjectId:", cultivationDataStep2);

    const cultivationData = await db.collection('cultivation').aggregate([
      { $match: { farmerid: new mongodb.ObjectId(farmerid) } },
      { $addFields: { subcategoryId: { $toObjectId: "$subcategory" } } },
      {
        $lookup: {
...
app.post('/deleteCultivation', (req, res) => {
  ...
  database.then(db => {
    db.collection('cultivation').deleteOne({ _id: new mongodb.ObjectId(cultivationId) })
      .then(result => {
        if (result.deletedCount === 1) {
          res.json({ status: "success" });
        } else {
          res.json({ status: "error", message: "Cultivation entry not found" });
        }
      })
      .catch(err => {
...
app.post('/updateCultivation', (req, res) => {
  ...
  database.then(db => {
    db.collection('cultivation').updateOne(
      ...
      .then(result => {
        if (result.modifiedCount === 1) {
          ...
          res.status(404).json({ status: "error", message: "Cultivation entry not found" });
        }
      })
      .catch(err => {
...
app.get('/getCultivation/:id', (req, res) => {
  ...
  database.then(async (db) => {
    try {
      ...
      var cultivationresult = await db.collection('cultivation').findOne({ _id: new mongodb.ObjectId(cultivationId) });
...
app.get('/viewproductsdata', (req, res) => {
  database.then(async (dbase) => {
    var productresult = await dbase.collection('product').aggregate([
      { "$addFields": { "categoryId": { "$toObjectId": "$item" } } },
      {
        $lookup: {
          from: 'category',
...
app.get('/singleproduct/:id', async (req, res) => {
  ...
  try {
    const db = await database;
    const productResult = await db.collection('product').findOne({ _id: new mongodb.ObjectId(findId) });
    const cultivationData = await db.collection('cultivation').findOne({ subcategory: new mongodb.ObjectId(findId) });
    const farmerid = productResult.farmerid;
    const farmerData = await db.collection('register').findOne({ userId: new mongodb.ObjectId(farmerid) });
    const ratings = await db.collection('ratings').find({ productId: new mongodb.ObjectId(findId) }).toArray();
...
app.post('/addcart', async (req, res) => {
  const { userid, productid, farmerid, status ,farmername, customerName} = req.body;
  ...
  try {
    ...
    console.log(result);
    res.json({ message: 'Item added to cart successfully', _id: result.insertedId });
  } catch (error) {
...
app.post('/viewcart', (req, res) => {
  ...
  database.then(async (dbase) => {
    var cartresult = await dbase.collection('cart').aggregate([
      { $match: { $and: [{ status: 0 }, { userid: new mongodb.ObjectId(userids) }] } }, // Convert userids to ObjectId
...
app.post('/deletecart', (req, res) => {
  let delId = req.body.id
  database.then((db) => {
    db.collection('cart').deleteOne({ _id: new mongodb.ObjectId(delId) })
      .then((result) => {
        res.json(result)
      }
      )
...
app.post('/verify', async (req, res) => {
  try {
    ...
    if (razorpay_signature === expectedSignature) {
      ...
      res.status(200).json({ message: "Payment verified and cart updated successfully" });
    } else {
...
app.post('/addfeedback', async (req, res) => {
  ...
  await database.then((dbase) => {
    dbase.collection('feedback').insertOne(feedbackData).then((results) => {
      console.log(results)
    })
  })
})
...
app.post('/feedback', (req, res) => {
  const farmerid = req.body.farmerid;
  database.then((db) => {
    db.collection('feedback').find({ farmer: farmerid }).toArray().then((result) => {
      console.log(result);
      res.json(result);
    }).catch((error) => {
...
app.get('/matchproduct/:id', async (req, res) => {
  try {
    ...
    database.then(async (db) => {
      ...
      const category = await db.collection('category').findOne({ _id: new mongodb.ObjectId(matchId) });
...
app.post('/orderlist', (req, res) => {
  ...
  database.then(async (dbase) => {
    var cartresult = await dbase.collection('cart').aggregate([
      { $match: { $and: [{ status: 1 }, { userid: new mongodb.ObjectId(userids) }] } }, // Convert userids to ObjectId
      { "$addFields": { "products": { "$toObjectId": "$productid" } } },
      {
        $lookup: {
          from: 'product',
...
app.post('/addcomplaints', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Complaint added successfully', _id: result.insertedId });
  } catch (error) {
...
app.post('/rating', async (req, res) => {
  const { productId, farmerId, userId, rating ,farmername,subcategory,username} = req.body;
...
app.post('/profile', async (req, res) => {
  ...
  try {
    ...
    const registerData = await db.collection('register').findOne({ userId: new mongodb.ObjectId(userid) });
    if (!registerData) {
      return res.status(404).json({ error: 'User not found' });
    }

    const loginData = await db.collection('login').findOne({ _id: new mongodb.ObjectId(registerData.userId) });
    if (!loginData) {
...
app.post('/updateProfile', async (req, res) => {
  ...
  try {
    ...
    const updatedProfile = await db.collection('register').findOne({ userId: new mongodb.ObjectId(userId) });
    const loginData = await db.collection('login').findOne({ _id: new mongodb.ObjectId(userId) });
...
app.get('/salelist', (req, res) => {
  database.then(async (dbase) => {
    try {
      ...
      var farmerLookupResult = await dbase.collection('register').aggregate([
        { $match: { userId: { $in: initialResult.map(item => item.farmerObjId) } } }
      ]).toArray();
      ...
      const mergedResult = initialResult.map(cartItem => {
        const farmerData = farmerLookupResult.find(farmer => farmer.userId.toString() === cartItem.farmerObjId.toString());
...
app.post('/salereport', (req, res) => {
  ...
  database.then(async (dbase) => {
    var cartresult = await dbase.collection('cart').aggregate([
      { $match: { $and: [{ status: 1 }, { farmerid: new mongodb.ObjectId(farmerid) }] } }, // Convert userids to ObjectId
      { "$addFields": { "products": { "$toObjectId": "$productid" } } },
      {
        $lookup: {
          from: 'product',
...
app.post('/updateproductstatus', (req, res) => {
  ...
  database.then((db) => {
      db.collection('product').updateOne({ _id: new mongodb.ObjectId(productId) }, { $set: { status: status } })
          .then(result => {
...
app.post('/bestfarmer', async (req, res) => {
  ...
  try {
    ...
    const farmer = await db.collection('register').findOne({ userId: new mongodb.ObjectId(topFarmerId )});
    res.json(farmer);
    console.log(farmer);
  } catch (error) {
